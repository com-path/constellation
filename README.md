...
- **A gentle way in** — the optional **setup journey** takes the sting out of an empty sky: pick names from your device (Contact Picker API or a `.vcf` file, parsed entirely in the browser — only names and birthdays are read), brain-dump people one per line with light syntax (`Sofia — work, close, loves ceramics, birthday 12-05`), then review everything on one screen before a single star lands. Imports default to the outer field — an address book is context, not closeness. The **sky census** follows: a deck of creative prompts ("think of a meal you'll never forget — who was around the table?") whose answers become real structure — stars, threads, moments, details — one card at a time, resumable forever via the nudges. And the sky always fills in flow: logging a moment with someone new places them on the spot.
...
| 7 | Import vs hand-placement | **Hand-placement, with staged on-ramps.** The "new star" flow takes under a minute; the optional setup journey (device contacts, brain-dump) and sky census bulk-stage people for review — nothing lands unconfirmed, imports default to the outer field, and sparseness stays a feature. |
